...
const printData = (data, sectionName) => {
    ...
    data.forEach(item => {
      let photo = item.Images;
        ...
        let titleItem = item.description.split(",");
        let template =`
        ...
        <button data-product-id=${item.listing_id} onclick="removeFromCart(${item.listing_id})" class='btn btn-primary'>Quitar del carrito</button>
